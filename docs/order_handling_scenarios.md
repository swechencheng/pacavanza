...
2. **API Request**: The frontend JavaScript (`future_chart.js`, `trading.js`) captures the input and sends a POST request to the corresponding backend endpoint (e.g., `/ibkr/buy_stop`).
...
5. **Event Subscription**: The backend listens to IBKR events (`orderStatusEvent`, `execDetailsEvent`). When an order executes or its status changes, it updates the local state and triggers synchronization logic.
...
* **Lifecycle**:
  ...
  * The `_on_exec_details` event fires, which triggers `_sync_sl_tp_volume()` to align any existing open stop-loss (SL) / take-profit (TP) order volumes with the new total position.
...
* **Backend Flow**: The backend queries the current signed position (`_get_signed_position()`). It then executes one of four distinct sub-scenarios:
  ...
  * **Scenario B3: Opposite Direction, Exact Match (Close Position)**
    ...
    * Places a **Standalone Stop Order** (`orderRef="CloseOnly"`).
...
* **Trigger**: Every time an order fills (`_on_exec_details`), the synchronization routine runs.
* **Logic**:
